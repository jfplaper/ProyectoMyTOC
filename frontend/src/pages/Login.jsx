        ...
        <form className="max-w-md mx-auto bg-white rounded-xl" onSubmit={handleSubmitLogin}>
          <div className="flex flex-col gap-2">
            ...
            <input className="p-2 mb-4 border-gray-200 border rounded-lg focus:outline-[#2AB7FA]" type="text"
              placeholder="Nombre de usuario" value={usernameLoginInput} onChange={(e) => setUsernameLoginInput(e.target.value)} />
            <input className="p-2 mb-4 border-gray-200 border rounded-lg focus:outline-[#2AB7FA]" type="password"
              placeholder="Password" value={passwordLoginInput} onChange={(e) => setPasswordLoginInput(e.target.value)} />
            <button className="w-full bg-[#2AB7FA] text-white text-xl rounded p-4 mb-6 hover:bg-blue-700 cursor-pointer" type="submit">
              Iniciar sesión
            </button>
            <Link className="font-bold center mx-auto" to="/register">
              *Si no te has registrado, <u className="text-[#2ABF7A]">hazlo aquí</u>
            </Link>
          </div>
        </form>
      </div>
